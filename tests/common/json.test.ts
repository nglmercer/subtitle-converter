 ...
 */
function createJsonContent(cues: Record<string, string | number | boolean>[]) {
 ...
 */
function cueToJsonCaption(
  cue: Record<string, string | number | boolean>,
  index: number,
) {
 ...
 */
function timeToMilliseconds(time: string | number): number {
...
describe("JSON Format", () => {
  describe("parseJson", () => {
    test("should parse valid JSON content", () => {
      const jsonCues = commonCues.map((cue, index) =>
        cueToJsonCaption(cue, index),
      );
    ...
    test("should handle multiline text", () => {
      const jsonCues = multilineCues.map((cue, index) =>
        cueToJsonCaption(cue, index),
      );
      const jsonContent = createJsonContent(jsonCues);
      const result = parseJson(jsonContent);
    ...
    test("should throw error for non-array JSON", () => {
      ...
      expect(() => parseJson(invalidJson)).toThrow(
        "JSON content must be an array of captions",
      );
    });
    ...
    test("should throw error for invalid start/end times", () => {
      ...
      expect(() => parseJson(jsonContent)).toThrow(
        "Caption 1 has invalid start/end times",
      );
    });
  ...
  describe("validateJsonStructure", () => {
    ...
    test("should detect overlapping cues", () => {
      const jsonCues = overlappingCues.map((cue, index) =>
        cueToJsonCaption(cue, index),
      );
      const jsonContent = createJsonContent(jsonCues);
      const result = validateJsonStructure(jsonContent);
    ...
    test("should detect empty text content", () => {
      ...
      expect(result.errors.some((error) => error.type === "EMPTY_CUE")).toBe(
        true,
      );
    });
    ...
    test("should warn about duration mismatch", () => {
      ...
      expect(result.warnings[0].message).toContain(
        "doesn't match calculated duration",
      );
    });
